...
public class goToNextMessage implements IPageEvent {

    private static final Logger log = LoggerFactory.getLogger(goToNextMessage.class);
    private final WhatsappMessageService whatsappMessageService;

    @Override
    public String getName() {
        return "goToNextMessage";
    }

    @Override
    public ProcessWhatsappMessageResponse processPageEvent(String msisdn, String country, WhatsappMessage message) {
        log.info("Invoke goToNextMessage event for msisdn : {} and country : {}", msisdn, country);
        whatsappMessageService.saveCustomerCurrentPage(msisdn, country, WhatsappMessageLabels.HI_USER_MESSAGE);
        return null;
    }
}
