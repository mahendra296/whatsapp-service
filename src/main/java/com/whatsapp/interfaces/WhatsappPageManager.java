...
public class WhatsappPageManager implements HandleWhatsappEventProvider {
    ...
    @Override
    public ProcessWhatsappMessageResponse processWhatsappRequest(
            String msisdn,
            String country,
        ...
        if (StringUtils.isEmpty(messageLabel)) {
            ...
            return this.handleWhatsappRequestStartEvent(
                    msisdn, country, messageLabel, inputMessage, inputButtonClick, inputMessageType, inputMediaUrl);
        }
        // must be a continue event
        return this.handleWhatsappRequestContinueEvent(
                msisdn, country, messageLabel, inputMessage, inputButtonClick, inputMessageType, inputMediaUrl);
    }

    public ProcessWhatsappMessageResponse handleWhatsappRequestStartEvent(
            String msisdn,
            String country,
            String messageLabel,
        ...
        ProcessWhatsappMessageResponse whatsappMessageResponse =
                fireMessageEvents(msisdn, country, whatsappMessage, WhatsappMessageEventType.PRE_INPUT);
    ...
    public ProcessWhatsappMessageResponse fireMessageEvents(
            String msisdn, String country, WhatsappMessage whatsappMessage, WhatsappMessageEventType eventType) {
        ...
        String eventListStr = (eventType == WhatsappMessageEventType.POST_INPUT)
                ? whatsappMessage.getEvents()
                : whatsappMessage.getPreInputEvents();
        ...
        for (String eventName : pageEventNames) {
            ...
            if (eventObject != null) {
                ...
                try {
                    ...
                    log.info("Method : " + e.request().httpMethod() + " Request Url : "
                            + e.request().url() + "\nstatus : "
                            + e.status() + "\nmessage : "
                            + e.getMessage());
    ...
    private ProcessWhatsappMessageResponse handleWhatsappRequestContinueEvent(
        ...
        if (whatsappMessage.getButtonActions() != null
            ...
            if (whatsappMessage.getEvents() != null && messageLabel != null && whatsappMessage.getDataLabel() != null) {
                whatsappMessageService.saveCustomerFormDataField(
                        msisdn, country, whatsappMessage.getDataLabel(), inputButtonClick);
                ProcessWhatsappMessageResponse fireMessageEvents =
            ...
            if (whatsappMessage.isSaveInputWithOptionType()) {
                if (whatsappMessage.getDataLabel() != null) {
                    whatsappMessageService.saveCustomerFormDataField(
                            msisdn,
                            country,
                            whatsappMessage.getDataLabel(),
                            whatsappInputMessage + "|" + inputMessageType + "|" + inputMediaUrl);
                }
            }
            return getResponseForNextPage(msisdn, country, messageLabel);
        }
        ...
        if (whatsappMessage.getType() == WhatsappMessageType.INPUT) {
            if (!whatsappMessage.getValidInputs().isEmpty()
                    && !whatsappMessage.getValidInputs().contains(whatsappInputMessage)) {
        ...
        if (whatsappMessage.getNextPageLabel() != null) {
            log.info("Changing current page using nextPageLabel config from : "
                    + whatsappMessageService.getCustomerCurrentPage(msisdn, country) + " to "
                    + whatsappMessage.getNextPageLabel());
    ...
    private ProcessWhatsappMessageResponse getResponseForNextPage(
            String msisdn, String country, String nextMessageLabel) {
        ...
        ProcessWhatsappMessageResponse whatsappMessageResponse =
                fireMessageEvents(msisdn, country, nextWhatsappMessage, WhatsappMessageEventType.PRE_INPUT);
        ...
        if (nextWhatsappMessage.isTerminal()) {
            ProcessWhatsappMessageResponse terminatePageResponse =
                    fireMessageEvents(msisdn, country, nextWhatsappMessage, WhatsappMessageEventType.POST_INPUT);
        ...
        return new ProcessWhatsappMessageResponse(
                msisdn, nextWhatsappMessage.getMessages(), nextWhatsappMessage.isTerminal());
    }

    public ProcessWhatsappMessageResponse wrongOptionSelectedMessage(String msisdn) {
    ...
    public ProcessWhatsappMessageResponse errorOccurredMessage(String msisdn, String label) {
