...
public class WhatsappController {
    ...
    List<WhatsappMediaType> mediaTypes =
            Arrays.asList(WhatsappMediaType.INTERACTIVE_BUTTON_REPLY, WhatsappMediaType.INTERACTIVE_LIST_REPLY);
    ...
    public InfoBipIncomingWhatsappResponse handleIncomingWhatsappMessage(
        ...
        for (Result input : request.getResults()) {
            ...
            processInputMessage(input, inputMessage, mediaType);
        }
    ...
    private void processInputMessage(Result input, String inputMessage, WhatsappMediaType mediaType) {
        try {
            ...
            // var code = COUNTRY_CODE_PREFIX + input.getFrom().substring(0, COUNTRY_CODE_LENGTH);
            var country = "india"; // Country.fromPhoneCode(code).name();

            var whatsappEventType = getEventType(inputMessage);
            ...
            processPageResponse.getMessages().forEach(message -> {
                try {
                    if (message.getActionButtons() != null
                            && !message.getActionButtons().isEmpty()) {
                        var infobipOutgoingButtonOutgoingDto =
                                prepareWhatsappButtonMessage(message, country, processPageResponse);
                        log.info(
                                "Sending interactive button message to Infobip: EventsType: {}",
                                infobipOutgoingButtonOutgoingDto);
                        ...
                        log.info(
                                "Sending interactive button message to Infobip: {}",
                                infobipInteractiveListReplyOutgoingDto);
                        sendInteractiveListMessage(infobipInteractiveListReplyOutgoingDto);
                        TimeUnit.MILLISECONDS.sleep(infoSendSmsDelayInSeconds);
                    } else if (message.getMediaType() == WhatsappMediaType.DOCUMENT) {
                        var infoBipDocumentOutgoingDto = prepareDocumentMessage(message, country, processPageResponse);
    ...
    private void sendDocumentMessage(InfoBipDocumentOutGoingDto infoBipDocumentOutgoingDto) {
        infobipWhatsappManagerAdapter.sendDocumentMessage(infoBipApiKey, infoBipDocumentOutgoingDto);
    }

    private InfoBipInteractiveOutGoingDto prepareInteractiveListMessage(
            MessageObject processedMsg, String country, ProcessWhatsappMessageResponse processPageResponse) {
        if (processedMsg.getInteractiveListMessage() == null
                || processedMsg.getInteractiveListMessage().getListSections().isEmpty()) {
        ...
        Content content = new Content(
                ...
                new WhatsappInteractiveButtonAction(
                        sections, processedMsg.getInteractiveListMessage().getTitle()),
                null,
                null);
        ...
        return new InfoBipInteractiveOutGoingDto(
                from != null ? from : "", processPageResponse.getMsisdn(), null, content);
    }

    private String getEventType(String inputMessage) {
        var whatsappEventType = AppConstants.CONTINUE_EVENT;
        if (getStartKeywords().contains(inputMessage.trim().toLowerCase())) {
            whatsappEventType = AppConstants.START_EVENT;
        }
        return whatsappEventType;
    }

    private List<String> getStartKeywords() {
        var startKeywords = "hi,hello,hey";
        return List.of(startKeywords.trim().toLowerCase().split(","));
    }

    public InfoBipInteractiveOutGoingDto prepareWhatsappButtonMessage(
            MessageObject processedMsg, String country, ProcessWhatsappMessageResponse processPageResponse) {
        ...
        return new InfoBipInteractiveOutGoingDto(from, processPageResponse.getMsisdn(), null, content);
    }
    ...
    public void sendInteractiveButtonMessage(InfoBipInteractiveOutGoingDto infobipInteractiveOutGoingdto) {
        APIResult<InfoBipOutgoingWhatsappResponse> result = infobipWhatsappManagerAdapter.sendInteractiveButtonMessage(
                infoBipApiKey, infobipInteractiveOutGoingdto);
    ...
    public void sendInteractiveListMessage(InfoBipInteractiveOutGoingDto infobipInteractiveOutGoingdto) {
        infobipWhatsappManagerAdapter.sendInteractiveListMessage(infoBipApiKey, infobipInteractiveOutGoingdto);
    }
    ...
    public InfoBipDocumentOutGoingDto prepareDocumentMessage(
        ...
        return new InfoBipDocumentOutGoingDto(
                from, processPageResponse.getMsisdn(), null, new DocumentContent(mediaUrl, filename, caption));
    }

    public void sendToInfobip(
            String country, ProcessWhatsappMessageResponse processPageResponse, WhatsApp whatsappOutgoingMessage) {
        ...
        InfoBipOutgoingWhatsappDto infobipOutgoingDto =
                new InfoBipOutgoingWhatsappDto(scenarioKey, destinationsList, whatsappOutgoingMessage);
